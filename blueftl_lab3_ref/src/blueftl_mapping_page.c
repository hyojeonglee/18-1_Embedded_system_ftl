	...
	uint32_t *ptr_page)
{
	...
	int32_t physical_page_address;
    /* mskim: pageoffset <= logical_page_address*/

	int32_t ret = -1;
	printf("-S----%s \n", __func__);
	printf("logical page address %u\n", logical_page_address);
	...
	)
{
	...
	if ((old_ppa!=-1) &&(new_ppa != old_ppa)) {
		...
		*/
		ptr_chunk_table[old_ppa / 32].valid_count--;
		
		//valid한 page가 없을 때, 압축된 모든 페이지를 invalidation
		if(ptr_chunk_table[old_ppa / 32].valid_count == 0){
			...
			for(i = 0; i < ptr_chunk_table[old_ppa / 32].physical_page_len; i++){
	...
	**/
	if(is_compressed){
		...
		for(i=0; i<nr_pages; i++){
			ptr_chunk_table[(ppa+i * 32) / 32].valid_count = WRITE_BUFFER_LEN;
			ptr_chunk_table[(ppa+i * 32) / 32].physical_page_len = nr_pages;
			ptr_chunk_table[(ppa+i * 32) / 32].is_compressed = 1;
			printf("compressed page: %d, valid_count: %d, physical_page_len:%d, is_compressed %d\n", ppa+i,ptr_chunk_table[(ppa+i * 32)/32].valid_count, ptr_chunk_table[(ppa+i*32)/32].physical_page_len, ptr_chunk_table[(ppa+i*32) / 32].is_compressed);
		}
	} else {
		...
		ptr_chunk_table[ppa / 32].valid_count = 1;
		ptr_chunk_table[ppa / 32].physical_page_len = 1;
		ptr_chunk_table[ppa / 32].is_compressed = 0;
		printf("no compressed page: %d, valid_count: %d, physical_page_len:%d, is_compressed %d\n", ppa,ptr_chunk_table[ppa/32].valid_count, ptr_chunk_table[ppa/32].physical_page_len, ptr_chunk_table[ppa / 32].is_compressed);
	}
