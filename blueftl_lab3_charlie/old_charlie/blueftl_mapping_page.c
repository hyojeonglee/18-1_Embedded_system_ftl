...
struct ftl_base_t ftl_base_page_mapping = {
	...
	.ftl_update_chunk_table = page_mapping_update_chunk_table,
		...
		struct virtual_device_t* ptr_vdevice)
{
	...
	for (init_pg_loop = 0; init_pg_loop < ptr_chunk_table->nr_chunk_table_entries; init_pg_loop++) {
		ptr_chunk_table->ptr_chunks[init_pg_loop].nr_valid_pages = 0;
		ptr_chunk_table->ptr_chunks[init_pg_loop].nr_physical_pages = 0;
...
{
	struct flash_ssd_t* ptr_ssd = ptr_ftl_context->ptr_ssd;
	struct ftl_chunk_table_context_t* ptr_chunk_table = (struct ftl_chunk_table_context_t*)ptr_ftl_context->ptr_chunk_table;

	uint32_t physical_page_address;
	int32_t ret = -1;

	physical_page_address = ftl_convert_to_physical_page_address (*ptr_bus, *ptr_chip, *ptr_block, *ptr_page);
	ptr_chunk_table->ptr_chunks[physical_page_address].nr_valid_pages = nr_valid_pages;
	ptr_chunk_table->ptr_chunks[physical_page_address].nr_physical_pages = nr_physical_pages;
	ptr_chunk_table->ptr_chunks[physical_page_address].is_compressed = is_compressed;
	return 0;
}
