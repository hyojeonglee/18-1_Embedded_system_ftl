	...
	uint32_t *ptr_page)
{
	...
	if (physical_page_address != PAGE_TABLE_FREE) {
		...
		if(ptr_erase_page->page_status != 2){ // 해당 page가 inavlid page가 아니면 지금 update 상황이므로 일단 예전걸 invalid 처리를 해주고
			ptr_erase_page->page_status = 2;
			ptr_erase_block->nr_valid_pages--;
			ptr_erase_block->nr_invalid_pages++;
		}

		/* 다시 요청 page를 위한 free page를 loop돌며 찾기 */
		int nr_all_used_block = 0;
		int found_free_page = 0; // Why does the this varible use? loop 돌면서 발견한 첫번째 page를 return하려고
		int is_all_used;
			...
			for (j = 0; j < ptr_ssd->nr_chips_per_bus; j++) {
				for (m = 0; m < ptr_ssd->nr_blocks_per_chip; m++) {
					...
					for (n = 0; n < ptr_ssd->nr_pages_per_block; n++) {
						if (ptr_erase_block->list_pages[n].page_status == PAGE_STATUS_FREE) { // free page 발견
				...
				//return -1;
			} // bus내 chip scanning is finished
		} // bus scanning is finished
		return -1;
	} else { // if (physical_page_address == PAGE_TABLE_FREE)  //해당 페이지가 처음으로 요청되는 경우 
		...
		if (physical_page_address == PAGE_TABLE_FREE) {
			if(ptr_ftl_context->latest_block == -1 || ptr_ssd->list_buses[ptr_ftl_context->latest_bus].list_chips[ptr_ftl_context->latest_chip].list_blocks[ptr_ftl_context->latest_block].nr_free_pages == 0){
				...
				if (ptr_erase_block == NULL) {  // What does it means? ssdmgmt 함수가 끝까지 다 도달했을 때?! => menually control needed
					for (i = 0 ; i < ptr_ssd->nr_buses; i++) {
						for (j = 0; j < ptr_ssd->nr_chips_per_bus; j++) {
							for (m = 0; m < ptr_ssd->nr_blocks_per_chip; m++) {
								ptr_erase_block = &ptr_ssd->list_buses[i].list_chips[j].list_blocks[m];
								...
								nr_all_used_block++;
								//ptr_erase_block->is_reserved_block = IS_RESERVED_FOR_GC; 
																			} // chip내 block scanning is finished.
							if (found_free_page == 1) {
								                       return 0;}
							
							else {
							                           return -1;}
						} // chip scanning is finished. 
					} // bus scanning is finished.
				}
			...
			}else {  // 아직 해당 blok내에 free page가 남아 있을 때 
			 ...
			 return 0; // 아직 해당 block내 free page가 남아있어서 for문 break하고 탈출했으므로 return 0;
		} // Close:  if (physical_page_address == PAGE_TABLE_FREE) 
	} //  Close:  if (physical_page_address == PAGE_TABLE_FREE) one more 
	return -1;
}
