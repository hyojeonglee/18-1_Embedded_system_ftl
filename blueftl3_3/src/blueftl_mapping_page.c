	...
	)
{
	...
	if ((old_ppa!=-1) &&(new_ppa != old_ppa)) {
		...
		*/
		ptr_chunk_table[rblock * 64 + rpage].valid_count--;
		
		//valid한 page가 없을 때, 압축된 모든 페이지를 invalidation
		if(ptr_chunk_table[rblock * 64 + rpage].valid_count == 0){
	//		ftl_convert_to_ssd_layout(old_ppa, &rbus, &rchip, &rblock, &rpage);
			old_block = &(ptr_ssd->list_buses[rbus].list_chips[rchip].list_blocks[rblock]);
			for(i = 0; i < ptr_chunk_table[rblock * 64 + page].physical_page_len; i++){
	...
	bool is_compressed)
{
	...
	**/
	if(is_compressed){
		...
		ptr_chunk_table[block * 64 + page].valid_count = 1;
		ptr_chunk_table[block * 64 + page].physical_page_len = 1;
		ptr_chunk_table[block * 64 + page].is_compressed = 0;
