...
struct c_header{
	uint32_t logical_page_address[CHUNK_SIZE];
	uint8_t offset[CHUNK_SIZE];
};

int find_data_in_chk (struct write_buffer_t* wb, uint32_t lpa){
	int i;
		...
		uint8_t* ptr_buffer)
{
	...
	if (_ftl_base.ftl_get_mapped_physical_page_address (
		...
		int count = find_data_in_chk (wb, lpa_curr);

		if (count != -1) {
			memcpy(ptr_lba_buff, wb->ptr_wb_buff + count * _ptr_vdevice->page_main_size, _ptr_vdevice->page_main_size);
		} else {
			...
			uint8_t* ptr_after_decomp = (uint8_t* )malloc( sizeof(struct c_header) + _ptr_vdevice->page_main_size * CHUNK_SIZE);
			memset(ptr_after_decomp, 0xFF, (sizeof(struct c_header) + _ptr_vdevice->page_main_size * CHUNK_SIZE));
			...
			if (is_comp) {
				...
				memcpy(temp_header, ptr_after_decomp, sizeof(struct c_header));

				for (i = 0; i < CHUNK_SIZE; i++) {
					if (temp_header->logical_page_address[i] == lpa_curr) {
						memcpy(ptr_lba_buff,  ptr_after_decomp + sizeof(struct c_header) + temp_header->offset[i]*_ptr_vdevice->page_main_size, _ptr_vdevice->page_main_size);
					}
				}
			}
			else {
				...
				if (wb->wb_page_cnt == CHUNK_SIZE) {
					...
					for(i = 0; i < wb->wb_page_cnt; i++) {
						if (ptr_pg_mapping->ptr_pg_table[wb->t_lpa[i]] != PAGE_TABLE_FREE) {
							_ftl_base.ftl_get_mapped_physical_page_address (_ptr_ftl_context, wb->t_lpa[i], &bus, &chip, &block, &page);
					...
					if ((ptr_commit_buff = (uint8_t*)malloc(sizeof(struct c_header) +  _ptr_vdevice->page_main_size * CHUNK_SIZE)) == NULL) {
						return -1;
					}
					struct c_header ch;
					for (i = 0; i < 4 ; i++) {
					...
					temp_wb_header = (uint8_t*)malloc(sizeof(struct c_header) +  _ptr_vdevice->page_main_size * CHUNK_SIZE);
					memcpy(temp_wb_header, &ch, sizeof(struct c_header));
					memcpy(temp_wb_header + sizeof(struct c_header), wb->ptr_wb_buff, _ptr_vdevice->page_main_size * CHUNK_SIZE);

					comp_res_size = compress(temp_wb_header, _ptr_vdevice->page_main_size * CHUNK_SIZE + sizeof(struct c_header) , ptr_commit_buff);
					free(temp_wb_header);
